...
//@ts-ignore
import { prepare, request as klipRequest, getResult as getKlipResult, getCardList } from "klip-sdk";

const MAIN_URL = "http://localhost:3000/"

export default function App() {
  const [result, setResult] = useState<any>("basic")

  const testFunc = async () => {
    try {
      const response = await prepare.auth({ bappName: "Sentudy", successLink: MAIN_URL, failLink: MAIN_URL})
      klipRequest(response.request_key)
      let timerId = setInterval(async () => {
        const data = await getKlipResult(response.request_key)
        if (data.status === "completed") {
          let klaytnAddress = data.result.klaytn_address
          setResult(klaytnAddress)
  ...
  return (
    <>
      <button onClick={async () => await testFunc()}>test</button>
